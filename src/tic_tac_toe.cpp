  ...
  bot.on_button_click([&bot](const dpp::button_click_t &event) {
    if (event.custom_id.starts_with("tic-tac-toe")) {
      ...
      bool end = false;
      {
        char winner = '\0';
        for (int i = 0; i < 3; i++) {
          if (field[0 + i * 3] == field[1 + i * 3] && field[1 + i * 3] == field[2 + i * 3] && field[0 + i * 3])
            winner = field[0 + i * 3];
          if (field[i + 0 * 3] == field[i + 1 * 3] && field[i + 1 * 3] == field[i + 2 * 3] && field[i + 0 * 3])
            winner = field[i + 0 * 3];
        }
        if (field[0 + 0 * 3] == field[1 + 1 * 3] && field[1 + 1 * 3] == field[2 + 2 * 3] && field[0 + 0 * 3])
          winner = field[0 + 0 * 3];
        if (field[2 + 0 * 3] == field[1 + 1 * 3] && field[1 + 1 * 3] == field[0 + 2 * 3] && field[2 + 0 * 3])
          winner = field[2 + 0 * 3];
